...
case $BRANCH in
  develop)
    docker build -t "$DOCKER_USER_NAME/foreverBE:DEV-$CODEBUILD_WEBHOOK_PREV_COMMIT" ../../.
    ;;
  main)
    docker build -t "$DOCKER_USER_NAME/foreverBE:PROD-$CODEBUILD_WEBHOOK_PREV_COMMIT" ../../.
    ;;
  *)
